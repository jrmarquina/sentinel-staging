...
const CAPABILITY_META: Record<string, { label: string; color: string }> = {
  ...
  org_manager:  { label: 'Gerente FM',    color: 'bg-blue-500/20 text-blue-300 border border-blue-500/30' },
  org_viewer:   { label: 'Director',      color: 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' },
  contributor:  { label: 'Colaborador',   color: 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' },
  worker:       { label: 'Trabajador',    color: 'bg-amber-500/20 text-amber-300 border border-amber-500/30' },
}
  ...
  if (member.is_me) {
    return (
      <span className={cn('text-xs px-2.5 py-1 rounded-full font-semibold', color)}>
        {displayName}
        <span className="ml-1 opacity-60">(tú)</span>
      </span>
    )
  }
...
function AddMemberModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!selectedDef) { setError('Selecciona un rol'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[#0d1b2e] border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <h2 className="text-sm font-bold text-white">Agregar Miembro FM</h2>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-5 space-y-4">
          ...
          {/* Role */}
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1">Rol en FM</label>
            <select
              value={roleDefId}
...
export function FmTeamClient({ members: initialMembers, roleDefs, isManager, currentUserId }: Props) {
  ...
  const groups: { label: string; items: Member[] }[] = [
    { label: 'Gestión',         items: managers },
    { label: 'Dirección',       items: directors },
    { label: 'Colaboradores',   items: contributors },
    { label: 'Trabajadores',    items: workers },
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-white">Equipo FM</h1>
          <p className="text-sm text-slate-400">
            {members.length} miembro{members.length !== 1 ? 's' : ''} · Head Start Facility Management
          </p>
        </div>
        {isManager && (
                ...
                <li key={member.user_id} className="flex items-center gap-4 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-white truncate">
                      {member.full_name}
                      {member.is_me && <span className="ml-2 text-xs font-normal text-slate-500">(tú)</span>}
                    </p>
                    {member.email && (
                      <p className="text-xs text-slate-500 mt-0.5 truncate">{member.email}</p>
                    )}
                    <p className="text-xs text-slate-600 mt-0.5">
                      Desde {new Date(member.joined_at).toLocaleDateString('es-PR', { month: 'short', year: 'numeric' })}
                    </p>
                  </div>
                  {isManager ? (
                    <CapabilityDropdown
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
        <div className="px-5 py-2.5 border-b border-white/5">
          <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Niveles de Acceso FM</p>
        </div>
        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          {[
            { cap: 'org_admin',   desc: 'Acceso total — gestión de equipo, configuración, todos los registros' },
            { cap: 'org_manager', desc: 'Gestión operacional completa — órdenes, inspecciones, asignaciones' },
            { cap: 'org_viewer',  desc: 'Lectura de todos los registros FM — sin modificaciones' },
            { cap: 'contributor', desc: 'Crear y gestionar sus propias solicitudes de mantenimiento' },
            { cap: 'worker',      desc: 'Ver y actualizar las órdenes de trabajo asignadas' },
          ].map(({ cap, desc }) => {
